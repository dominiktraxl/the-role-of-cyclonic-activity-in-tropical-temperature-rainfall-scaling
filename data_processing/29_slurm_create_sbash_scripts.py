...
# slurm folders
if cg_N is not None:
    slurmSBFiles = f'gl_cg_{cg_N}_sxt_vs_r{tcfstr}_parts_SBFiles/'
    slurmOUT = f'gl_cg_{cg_N}_sxt_vs_r{tcfstr}_parts_OUTPUT/'
    slurmSTDERR = f'gl_cg_{cg_N}_sxt_vs_r{tcfstr}_parts_STDERR/'
else:
    slurmSBFiles = f'gl_sxt_vs_r{tcfstr}_parts_SBFiles/'
    slurmOUT = f'gl_sxt_vs_r{tcfstr}_parts_OUTPUT/'
    slurmSTDERR = f'gl_sxt_vs_r{tcfstr}_parts_STDERR/'
...
# bursts to compute gl fit parts for
if cg_N is not None:
...
def _generate_sbatch_commandfile(locs, sbfile):
    ...
    if cg_N is not None:
        ...
        f.write('#SBATCH --mem-per-cpu=500M\n')
...
# write and execute
# locs = locs[:5]  # locs[:n_jobs*n_locs_per_job]
